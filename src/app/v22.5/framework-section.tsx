...
export default function FrameworkSection() {
  ...
  useEffect(() => {
    ...
    /* Hardened reveal logic — three layers of protection against the cards
       getting stuck at opacity 0:
       1. Synchronous initial check: if the section is already in the viewport
          on mount (deep-link / hash navigation / hydrated mid-section), reveal
          immediately without waiting for the IO callback.
       2. IntersectionObserver with threshold:0, no rootMargin shrinkage —
          fires on the first pixel of intersection. Handles normal scroll-in.
       3. Fallback timer (2s): if neither of the above fired (rare race
          conditions on slow mobile or aggressive prefetch), force-reveal so
          the section is never permanently hidden. */
