...
function LogoTile({ logo }: { logo: Logo }) {
  ...
  if (logo.kind === 'wordmark') {
    return (
      <div className='v20-logo-tile'>
        <span className='v20-tile-wm' style={style}>
          {logo.name}
        </span>
        {logo.caption ? <span className='v20-tile-cap'>{logo.caption}</span> : null}
      </div>
    )
  }
  ...
  if (logo.kind === 'mark-text') {
    return (
      <div className='v20-logo-tile'>
        <span className='v20-tile-wm v20-tile-with-mark' style={style}>
          {logo.mark ? <Icon icon={logo.mark} width={18} /> : null}
          <span>
            {logo.name}
            {logo.suffix ? <span className='v20-tile-suf'>{logo.suffix}</span> : null}
          </span>
        </span>
      </div>
    )
  ...
  if (logo.kind === 'stacked') {
    return (
      <div className='v20-logo-tile'>
        <span className='v20-tile-wm v20-tile-stacked' style={style}>
          <span className='v20-tile-top'>{logo.name}</span>
          <span className='v20-tile-bot'>{logo.suffix}</span>
        </span>
      </div>
    )
  }
