...
export default function ProvenSection() {
  ...
  return (
    <section className='v21-section v21-proven' id='work'>
      <div className='v21-container'>
        {/* Header row */}
        <div className='v21-proven-header'>
          <div className='v21-proven-header-left'>
            <div className='num'>N°05 / Proven</div>
            <h2>Proven where it <em>matters.</em></h2>
          </div>
          <div className='v21-proven-header-right'>
        ...
        <div className='v21-proven-filters'>
          {(['all', 'industry', 'solution', 'technology'] as Filter[]).map((f) => (
            <button
              key={f}
              className={`v21-proven-filter ${filter === f ? 'active' : ''}`}
              onClick={() => setFilter(f)}
            >
        ...
        {/* Carousel */}
        <div className='v21-proven-carousel'>
          <div className='v21-proven-track' ref={trackRef}>
            {filtered.map((c, i) => (
              <div
                key={c.title}
                className={`v21-proven-card ${i === activeIdx ? 'active' : ''}`}
                onClick={() => setActiveIdx(i)}
              >
                {/* Image area */}
                <div className='v21-proven-img'>
                  <img src={c.image} alt={c.kicker} loading='lazy' />
                  <div className='v21-proven-img-overlay'>
                    <span className='v21-proven-metric'>{c.metric}</span>
                  </div>
                  <div className='v21-proven-img-dots' />
                </div>
                {/* Content */}
                <div className='v21-proven-content'>
                  <span className='v21-proven-kicker'>{c.kicker}</span>
                  <h4>{c.title}</h4>
                  <p>{c.body}</p>
                  {c.techs && (
                    <div className='v21-proven-techs'>
                      {c.techs.map((t) => <span key={t} className='v21-proven-tech-pill'>{t}</span>)}
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* Navigation + CTA row */}
          <div className='v21-proven-nav'>
            <div className='v21-proven-nav-center'>
              <button className='v21-proven-arrow' onClick={prev} aria-label='Previous'>←</button>
              <div className='v21-proven-dots'>
                {filtered.map((_, i) => (
                  <span key={i} className={`v21-proven-dot ${i === activeIdx ? 'active' : ''}`} onClick={() => setActiveIdx(i)} />
                ))}
              </div>
              <button className='v21-proven-arrow' onClick={next} aria-label='Next'>→</button>
            </div>
            <a href='/case-studies' className='v21-proven-all-cta'>
