...
export default function TestimonialsSection() {
  ...
  return (
    <section className='v21-quote-section' data-surface='light'>
      <div className='v21-container'>
        <div className='v21-quote-frame' key={active}>
          <div className='v21-quote-meta'>
            <span>N°04 / Testimonial</span>
            <span>{t.company.toUpperCase()}</span>
          </div>
          <div className='v21-quote-mark' />
          <blockquote>{t.quote}</blockquote>
          <cite>{t.name} &nbsp;&mdash;&nbsp; {t.role}, {t.company}</cite>
        </div>
        <div className='v21-quote-nav'>
          <button className='v21-quote-arrow' onClick={() => setActive((active - 1 + testimonials.length) % testimonials.length)} aria-label='Previous'>←</button>
          <div className='v21-quote-dots'>
            {testimonials.map((_, i) => (
              <button key={i} className={`v21-quote-dot ${i === active ? 'active' : ''}`} onClick={() => setActive(i)} aria-label={`Testimonial ${i + 1}`} />
            ))}
          </div>
          <button className='v21-quote-arrow' onClick={() => setActive((active + 1) % testimonials.length)} aria-label='Next'>→</button>
        </div>
      </div>
    </section>
  )
