...
function WhyDiagram({ active, onSelect }: { active: number; onSelect: (i: number) => void }) {
  ...
  return (
    <svg viewBox='0 0 440 440' xmlns='http://www.w3.org/2000/svg' className='v21-why-diagram'>
      <defs>
        <filter id='v21-why-glow'>
          <feGaussianBlur stdDeviation='4' result='coloredBlur' />
          <feMerge><feMergeNode in='coloredBlur' /><feMergeNode in='SourceGraphic' /></feMerge>
        </filter>
        <filter id='v21-why-glow-sm'>
          <feGaussianBlur stdDeviation='2' result='coloredBlur' />
          <feMerge><feMergeNode in='coloredBlur' /><feMergeNode in='SourceGraphic' /></feMerge>
        </filter>
        <clipPath id='v21-why-clip'><rect x='12' y='12' width='416' height='416' /></clipPath>
      </defs>
      ...
      <g clipPath='url(#v21-why-clip)'>
        ...
        <circle cx={cx} cy={cy} r={r} fill='none' stroke='rgba(240,123,47,0.18)' strokeWidth='0.8' strokeDasharray='5 4' className='v21-why-orbit' />
        ...
        {nodes.map((node) => {
          ...
          return (
            <g key={`spoke-${node.idx}`}>
              <line x1={sx} y1={sy} x2={ex} y2={ey} stroke='#F07B2F' strokeWidth={isActive ? 1.5 : 0.8} strokeDasharray='5 4' opacity={isActive ? 0.9 : 0.25} className='v21-why-spoke' />
              <circle r='2' fill='#F07B2F' opacity={isActive ? 0.9 : 0.2} filter='url(#v21-why-glow-sm)'>
        ...
        {nodes.map((node) => {
          const isActive = node.i === active
          return (
            <g key={node.idx} className={`v21-why-node ${isActive ? 'active' : ''}`} onClick={() => onSelect(node.i)} style={{ cursor: 'pointer' }}>
              {isActive && <circle cx={node.x} cy={node.y} r={nodeR + 6} fill='none' stroke='#F07B2F' strokeWidth='0.8' opacity='0.4' strokeDasharray='3 3' className='v21-why-active-ring' />}
              <circle cx={node.x} cy={node.y} r={nodeR} fill={isActive ? '#F07B2F' : 'rgba(13,27,62,0.85)'} stroke={isActive ? '#F07B2F' : '#1A2D5A'} strokeWidth={isActive ? 2 : 1.5} filter={isActive ? 'url(#v21-why-glow)' : 'none'} className='v21-why-node-circle' />
        ...
        <circle cx={cx} cy={cy} r={hubR + 6} fill='none' stroke='rgba(240,123,47,0.12)' strokeWidth='0.8' strokeDasharray='3 3' className='v21-why-active-ring' />
        <circle cx={cx} cy={cy} r='18' fill='#E06B1F' className='v21-why-hub-pulse' />
...
export default function WhySection() {
  ...
  return (
    <section className='v21-section' id='about' data-surface='light'>
      <div className='v21-container'>
        <div className='v21-why-head'>
        ...
        <div className='v21-why-interactive'>
          {/* Diagram — center */}
          <div className='v21-why-diagram-wrap'>
            <WhyDiagram active={active} onSelect={setActive} />
          </div>

          {/* Detail panel — right */}
          <div className='v21-why-detail' key={active}>
            <span className='v21-why-detail-idx'>{reason.idx} / {reason.label}</span>
            <h3>{reason.title}</h3>
            <p>{reason.body}</p>
            <a href='#cta' className='v21-why-cta'>Learn more about us <span className='arrow'>→</span></a>
            <div className='v21-why-detail-nav'>
              {reasons.map((_, i) => (
                <button
                  key={i}
                  className={`v21-why-detail-dot ${i === active ? 'active' : ''}`}
                  onClick={() => setActive(i)}
