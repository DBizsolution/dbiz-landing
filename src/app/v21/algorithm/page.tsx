  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '0.2s' }}>
      ...
      <path d={`M ${tipX} ${AXIS_Y} L ${baseX} ${AXIS_Y - r} L ${baseX} ${AXIS_Y + r} Z`}
        fill='var(--v21-paper-2)' stroke={INK_HI} strokeWidth='1' />
      {/* body — top edge */}
      ...
      {/* front cap (right) */}
      <Cap cx={tailX} cy={AXIS_Y} r={r} stroke={INK_HI} fill='var(--v21-paper-2)' />
...
function HexPrism({ cx }: { cx: number }) {
  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '0.5s' }}>
      ...
      <polygon points={back.map(p => p.join(',')).join(' ')} fill='var(--v21-paper-2)' stroke={INK_LO} strokeWidth='0.8' strokeDasharray='3 2' />
      ...
      {/* front hex */}
      <polygon points={front.map(p => p.join(',')).join(' ')} fill='var(--v21-paper-2)' stroke={INK_HI} strokeWidth='1' />
      ...
      {front.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r='2' fill='var(--v21-paper)' stroke={INK_HI} strokeWidth='0.6' />
      ))}
      {/* center bore */}
...
function CloudDrum({ cx }: { cx: number }) {
  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '0.8s' }}>
      {slices.map((s, i) => {
        ...
        return (
          <g key={i}>
            ...
            <line x1={s.x} y1={AXIS_Y + r} x2={s.x + s.w} y2={AXIS_Y + r} stroke={s.color} strokeWidth={isAcc ? 1.2 : 0.9} />
            {/* front cap */}
            <Cap cx={s.x + s.w} cy={AXIS_Y} r={r} stroke={s.color} sw={isAcc ? 1.2 : 0.9} fill='var(--v21-paper-2)' />
      ...
      <Cap cx={cx + 44} cy={AXIS_Y} r={r * 0.6} stroke={INK_HI} fill='var(--v21-paper-2)' />
...
function Turbine({ cx }: { cx: number }) {
  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '1.1s' }}>
      ...
      <line x1={x1 - 14} y1={AXIS_Y + 8} x2={x1} y2={AXIS_Y + 8} stroke={INK_HI} strokeWidth='0.9' />
      <Cap cx={x1} cy={AXIS_Y} r={8} stroke={INK_HI} fill='var(--v21-paper-2)' />
      ...
      <line x1={x1} y1={AXIS_Y + r} x2={x2} y2={AXIS_Y + r} stroke={INK_HI} strokeWidth='1' />
      <Cap cx={x2} cy={AXIS_Y} r={r} stroke={INK_HI} fill='var(--v21-paper-2)' />
...
function DualRotor({ cx }: { cx: number }) {
  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '1.4s' }}>
      {/* axis sleeve */}
      <Cap cx={cx - 50} cy={AXIS_Y} r={6} stroke={INK_HI} fill='var(--v21-paper-2)' />
      <line x1={cx - 50} y1={AXIS_Y - 6} x2={cx - 32} y2={AXIS_Y - 6} stroke={INK_HI} strokeWidth='0.8' />
      <line x1={cx - 50} y1={AXIS_Y + 6} x2={cx - 32} y2={AXIS_Y + 6} stroke={INK_HI} strokeWidth='0.8' />

      {/* left rotor disc */}
      <Cap cx={xL} cy={AXIS_Y} r={r} dashed stroke={INK_LO} />
      <Cap cx={xL + 6} cy={AXIS_Y} r={r} stroke={INK_MID} sw={1} fill='var(--v21-paper-2)' />
      {/* right rotor disc — orange */}
      <Cap cx={xR} cy={AXIS_Y} r={r} dashed stroke={INK_LO} />
      <Cap cx={xR + 6} cy={AXIS_Y} r={r} stroke={ACC} sw={1.2} fill='var(--v21-paper-2)' />
...
function HubSpokes({ cx }: { cx: number }) {
  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '1.7s' }}>
      ...
      {/* hub sphere */}
      <circle cx={cx} cy={AXIS_Y} r='10' fill='var(--v21-paper-2)' stroke={ACC} strokeWidth='1.4' />
      ...
      {Array.from({ length: nodes }).map((_, i) => {
        ...
        return (
          <g key={i}>
            ...
            <circle cx={nx} cy={ny} r='4.2' fill={isActive ? ACC : 'var(--v21-paper-2)'} stroke={ACC} strokeWidth={isActive ? 0 : 1.1}>
              {isActive && (
...
function DataBrick({ cx }: { cx: number }) {
  ...
  return (
    <g className='alg-seg' style={{ animationDelay: '2.0s' }}>
      ...
      <rect x={x1 + dx} y={y1 + dy} width={w} height={h} fill='var(--v21-paper-2)' stroke={INK_LO} strokeWidth='0.6' strokeDasharray='3 2' />
      {/* connecting edges */}
      ...
      <polygon points={`${x1},${y1} ${x2},${y1} ${x2 + dx},${y1 + dy} ${x1 + dx},${y1 + dy}`}
        fill='var(--v21-paper)' stroke={INK_HI} strokeWidth='0.9' />
      {/* right face */}
      <polygon points={`${x2},${y1} ${x2 + dx},${y1 + dy} ${x2 + dx},${y2 + dy} ${x2},${y2}`}
        fill='var(--v21-paper-2)' stroke={INK_HI} strokeWidth='0.9' />
      {/* front face */}
      <rect x={x1} y={y1} width={w} height={h} fill='var(--v21-paper-2)' stroke={INK_HI} strokeWidth='1' />
...
function AlgorithmIllustration() {
  return (
    <svg viewBox={`0 0 ${W} ${H}`} xmlns='http://www.w3.org/2000/svg'
      ...
      {/* header strip */}
      <text x='50' y='34' fontFamily='var(--font-mono)' fontSize='9' fill='var(--v21-ink-label-strong)' letterSpacing='2'>SCALE 1:1</text>
      <line x1={W / 2 - 320} y1='44' x2={W / 2 + 320} y2='44' stroke='var(--v21-ink-corner)' strokeWidth='0.8' />
      <line x1={W / 2 - 320} y1='40' x2={W / 2 - 320} y2='48' stroke='var(--v21-ink-corner)' strokeWidth='0.8' />
      <line x1={W / 2 + 320} y1='40' x2={W / 2 + 320} y2='48' stroke='var(--v21-ink-corner)' strokeWidth='0.8' />
      <text x={W / 2} y='34' fontFamily='var(--font-mono)' fontSize='10' fill='var(--v21-ink-dim)' textAnchor='middle' letterSpacing='3'>
        ALGORITHM · SEVEN-PART ASSEMBLY · HUMAN-LED · AGENT-OPERATED · DATA-POWERED
      </text>
      <text x={W - 50} y='34' fontFamily='var(--font-mono)' fontSize='9' fill='var(--v21-ink-label-strong)' textAnchor='end' letterSpacing='1'>SHEET A2</text>

      {/* dimension brackets — top */}
      <g stroke='var(--v21-ink-corner)' strokeWidth='0.6' fill='none'>
        <line x1={CXS[0]} y1='64' x2={CXS[6]} y2='64' />
        <line x1={CXS[0]} y1='60' x2={CXS[0]} y2='68' />
        <line x1={CXS[6]} y1='60' x2={CXS[6]} y2='68' />
      </g>
      <text x={W / 2} y='80' fontFamily='var(--font-mono)' fontSize='9' fill='var(--v21-ink-label)' textAnchor='middle' letterSpacing='2.5'>
      ...
      {/* axis end pins */}
      <line x1='80' y1={AXIS_Y - 10} x2='80' y2={AXIS_Y + 10} stroke='var(--v21-ink-corner)' strokeWidth='1' />
      <line x1={W - 80} y1={AXIS_Y - 10} x2={W - 80} y2={AXIS_Y + 10} stroke='var(--v21-ink-corner)' strokeWidth='1' />
      ...
      {SEGMENTS.map((seg, i) => {
        ...
        return (
          <g key={seg.code}>
            {/* layer code (above) */}
            <text x={cx} y={AXIS_Y - 130} fontFamily='var(--font-mono)' fontSize='10' fill='var(--v21-ink-label)'
      ...
      <text x={(CXS[0] + CXS[2]) / 2} y={H - 50} fontFamily='var(--font-mono)' fontSize='9' fill='var(--v21-ink-label)' textAnchor='middle' letterSpacing='2'>FOUNDATION</text>
      <text x={(CXS[3] + CXS[6]) / 2} y={H - 50} fontFamily='var(--font-mono)' fontSize='9' fill='var(--v21-ink-label)' textAnchor='middle' letterSpacing='2'>ACTIVATION</text>
      ...
      {/* footer */}
      <text x={W / 2} y={H - 26} fontFamily='var(--font-mono)' fontSize='9' fill='var(--v21-ink-label-strong)' textAnchor='middle' letterSpacing='2'>
    ...
    <main className='alg-page'>
      <style>{`
        ...
        .alg-frame {
          ...
          margin: 0 auto;
          background: var(--v21-paper);
          border: 1px solid var(--v21-ink-frame);
          border-radius: 4px;
          padding: 4px;
          overflow-x: auto;
