...
function CardGlyph({ i }: { i: number }) {
  ...
  switch (i) {
    case 0: // EXPERTISE — compass with rotating bezel + lock-on dot
      return (
        <svg viewBox='0 0 120 120' aria-hidden='true' className='v22alt-why-glyph-svg g0'>
    ...
    case 1: // SCALE — concentric hexes + sonar pulse
      return (
        <svg viewBox='0 0 120 120' aria-hidden='true' className='v22alt-why-glyph-svg g1'>
    ...
    case 2: // DELIVERY — calibrated timeline with advancing arrow
      return (
        <svg viewBox='0 0 120 120' aria-hidden='true' className='v22alt-why-glyph-svg g2'>
    ...
    case 3: // PLATFORMS — stack assembly, layers settle in
      return (
        <svg viewBox='0 0 120 120' aria-hidden='true' className='v22alt-why-glyph-svg g3'>
    ...
    case 4: // PARTNERS — hub + four nodes with sequential ping
      return (
        <svg viewBox='0 0 120 120' aria-hidden='true' className='v22alt-why-glyph-svg g4'>
    ...
    default:
      return (
        <svg viewBox='0 0 120 120' aria-hidden='true' className='v22alt-why-glyph-svg g5'>
...
export default function WhySection() {
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      const cards = rail.querySelectorAll<HTMLElement>('.v22alt-why-card')
      let bestIdx = 0
  ...
  return (
    <section className='v22alt-section' id='about' data-surface='light'>
      <div className='v22alt-container'>
        <div className='v22alt-why-head' ref={revealRef}>
        ...
        <div className={`v22alt-why-rail-wrap ${visible ? 'is-visible' : ''}`}>
          <div className='v22alt-why-rail' ref={railRef} role='region' aria-label='DBiz differentiators'>
            {reasons.map((r, i) => (
              <article
                key={r.idx}
                className={`v22alt-why-card v22alt-why-paper-${i} ${active === i ? 'is-active' : ''}`}
                ...
                onFocus={() => scrollTo(i)}
              >
                <span className='v22alt-why-card-corner tl' aria-hidden='true' />
                <span className='v22alt-why-card-corner tr' aria-hidden='true' />
                <span className='v22alt-why-card-corner bl' aria-hidden='true' />
                <span className='v22alt-why-card-corner br' aria-hidden='true' />

                <div className='v22alt-why-card-stage' aria-hidden='true'>
                  <span className='v22alt-why-card-guide v-line' />
                  <span className='v22alt-why-card-guide h-line' />
                  <CardGlyph i={i} />
                </div>

                <div className='v22alt-why-card-meta'>
                  <span className='v22alt-why-card-idx'>
                    <CountUp target={Number(r.idx)} visible={visible} durationMs={700 + i * 80} />
                  </span>
                  <span className='v22alt-why-card-label'>{r.label}</span>
                </div>

                <div className='v22alt-why-card-rule' aria-hidden='true' />

                <h3 className='v22alt-why-card-title'>{r.title}</h3>
                <p className='v22alt-why-card-body'>{r.body}</p>

                <span className='v22alt-why-card-foot'>{r.meta}</span>
              </article>
            ))}
          </div>

          <div className='v22alt-why-rail-fade v22alt-why-rail-fade-l' aria-hidden='true' />
          <div className='v22alt-why-rail-fade v22alt-why-rail-fade-r' aria-hidden='true' />
        </div>

        <div className='v22alt-why-controls'>
          <button
            className='v22alt-why-nav'
            type='button'
          ...
          <div className='v22alt-why-progress' aria-hidden='true'>
            <div className='v22alt-why-progress-fill' style={{ transform: `scaleX(${progress})` }} />
            <div className='v22alt-why-progress-stops'>
              {reasons.map((_, i) => (
                <button
                  key={i}
                  type='button'
                  className={`v22alt-why-progress-dot ${active >= i ? 'is-passed' : ''} ${active === i ? 'is-current' : ''}`}
                  onClick={() => scrollTo(i)}
          ...
          <span className='v22alt-why-counter'>
            <span className='v22alt-why-counter-now'>{String(active + 1).padStart(2, '0')}</span>
            <span className='v22alt-why-counter-sep'>/</span>
            <span className='v22alt-why-counter-total'>{String(reasons.length).padStart(2, '0')}</span>
          </span>
        </div>
      </div>
    </section>
