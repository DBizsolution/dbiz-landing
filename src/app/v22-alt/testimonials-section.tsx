...
export default function TestimonialsSection() {
  ...
  return (
    <section className='v22alt-quote-section' data-surface='light'>
      <div className='v22alt-container'>
        <div className='v22alt-quote-frame' key={active}>
          <div className='v22alt-quote-meta'>
            <span>N°05 / Testimonial</span>
            <span>{t.company.toUpperCase()}</span>
          </div>
          <div className='v22alt-quote-mark' />
          <blockquote>{t.quote}</blockquote>
          <cite>{t.name} &nbsp;&mdash;&nbsp; {t.role}, {t.company}</cite>
        </div>
        <div className='v22alt-quote-nav'>
          <button className='v22alt-quote-arrow' onClick={() => setActive((active - 1 + testimonials.length) % testimonials.length)} aria-label='Previous'>←</button>
          <div className='v22alt-quote-dots'>
            {testimonials.map((_, i) => (
              <button key={i} className={`v22alt-quote-dot ${i === active ? 'active' : ''}`} onClick={() => setActive(i)} aria-label={`Testimonial ${i + 1}`} />
            ))}
          </div>
          <button className='v22alt-quote-arrow' onClick={() => setActive((active + 1) % testimonials.length)} aria-label='Next'>→</button>
        </div>
      </div>
    </section>
  )
