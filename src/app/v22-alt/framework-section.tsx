...
export function MiniDisc({ i }: { i: number }) {
  ...
  const r = 60
  return (
    <svg viewBox='0 0 160 160' xmlns='http://www.w3.org/2000/svg' className='v22alt-fw-mini-disc' aria-hidden='true'>
      <defs>
        <pattern id={`v22alt-mini-grid-${i}`} patternUnits='userSpaceOnUse' width='12' height='12'>
      ...
      <rect x={cx - r + 14} y={cy - r + 14} width={(r - 14) * 2} height={(r - 14) * 2} fill={`url(#v22alt-mini-grid-${i})`} />
      {renderShape(i, cx, cy)}
    </svg>
  )
}
...
function StackDiagram({ hovered, onHover }: { hovered: number; onHover: (i: number) => void }) {
  ...
  return (
    <svg
      ...
      className='v22alt-fw-diagram'
      aria-label='DBiz Transformation Assembly — Exploded View'
    >
      <defs>
        <pattern id='v22alt-fw-ex-dot' patternUnits='userSpaceOnUse' width='14' height='14'>
          <circle cx='1' cy='1' r='0.6' fill='rgba(240,123,47,0.14)' />
        </pattern>
        <pattern id='v22alt-fw-disc-grid' patternUnits='userSpaceOnUse' width='14' height='14'>
          <path d='M 14 0 L 0 0 0 14' fill='none' stroke='rgba(255,255,255,0.14)' strokeWidth='0.6' />
        </pattern>
        <filter id='v22alt-fw-halo-blur' x='-30%' y='-30%' width='160%' height='160%'>
          <feGaussianBlur stdDeviation='14' />
        </filter>
        <radialGradient id='v22alt-fw-disc-halo'>
          <stop offset='0%' stopColor='var(--v22alt-accent)' stopOpacity='0.85' />
          <stop offset='35%' stopColor='var(--v22alt-accent)' stopOpacity='0.45' />
          <stop offset='70%' stopColor='var(--v22alt-accent)' stopOpacity='0.15' />
          <stop offset='100%' stopColor='var(--v22alt-accent)' stopOpacity='0' />
        </radialGradient>
      </defs>

      {/* Orange dot background */}
      <rect x='8' y='8' width={svgW - 16} height={svgH - 16} fill='url(#v22alt-fw-ex-dot)' />
      ...
      {layers.map((layer, i) => {
        ...
        return (
          <g
            key={layer.n}
            className={`v22alt-fw-stack-layer ${isActive ? 'is-active' : ''}`}
          ...
          >
            ...
            <rect x={cx - discR - 14} y={shaftY - discR - 14} width={(discR + 14) * 2} height={(discR + 14) * 2} fill='var(--v22alt-accent)' fillOpacity='0.35' filter='url(#v22alt-fw-halo-blur)' className='v22alt-fw-disc-halo' style={{ animationDelay: `${i * 1.4}s` } as React.CSSProperties} />
            ...
            <rect x={cx - discR + 15} y={shaftY - discR + 15} width={(discR - 15) * 2} height={(discR - 15) * 2} fill='url(#v22alt-fw-disc-grid)' />
            ...
            <circle cx={cx} cy={shaftY} r='2.5' fill='var(--v22alt-accent)' />
            ...
            <line x1={cx} y1={layerNameY + 8} x2={cx} y2={svgH - 4} stroke='rgba(255,255,255,0.22)' strokeWidth='0.8' strokeDasharray='3 3' className='v22alt-fw-stack-connector' />
            <circle cx={cx} cy={svgH - 4} r='2.8' fill='var(--v22alt-accent)' className='v22alt-fw-stack-dot' />
          </g>
        )
      })}
...
export default function FrameworkSection() {
  ...
  return (
    <section className='v22alt-section v22alt-framework' id='framework'>
      <div className='v22alt-container'>
        <div className='v22alt-section-head v22alt-fw-head'>
          <div className='num'>N°04 / How we do it</div>
          <h2>
            Our AI Transformation Stack, <span style={{ color: 'var(--v22alt-accent)' }}>deconstructed.</span>
          </h2>
          <p className='lead'>
        ...
        <div className='v22alt-fw-top-strip' aria-hidden='true'>
          <span className='v22alt-fw-top-code'>SCALE 1:1</span>
          <span className='v22alt-fw-top-promise'>FULL STACK · NO CAPABILITY GAPS · NO VENDOR LOCK-IN</span>
          <span className='v22alt-fw-top-code'>SHEET A1</span>
        </div>

        <div className='v22alt-fw-scroller is-at-start' ref={scrollerRef}>
        <span className='v22alt-fw-scroll-edge left' aria-hidden='true' />
        <span className='v22alt-fw-scroll-edge right' aria-hidden='true' />
        <div className='v22alt-fw-scroll-hint' aria-hidden='true'>
          <span className='v22alt-fw-scroll-hint-arrow'>↔</span>
          <span>Drag to explore · 7 layers</span>
        </div>
        <div className='v22alt-fw-diagram-wrap'>
          <StackDiagram hovered={hovered} onHover={setHovered} />
        </div>

        <div
          ref={cardsRef}
          className={`v22alt-fw-cards ${visible ? 'is-visible' : ''} ${hovered >= 0 ? 'has-hover' : ''}`}
          aria-label='Stack layers — offerings and accelerators'
        >
          {layers.map((layer, i) => (
            <article
              key={layer.n}
              className={`v22alt-fw-card ${hovered === i ? 'is-active' : ''}`}
              style={{
                ['--card-i' as string]: i,
            ...
            >
              <div className='v22alt-fw-card-mini' aria-hidden='true'>
                <MiniDisc i={i} />
                <div className='v22alt-fw-card-layer'>L{layer.n} · {layer.label}</div>
              </div>
              <div className='v22alt-fw-card-item'>
                <div className='v22alt-fw-card-name'><TwoLine text={layer.top.name} /></div>
                <p className='v22alt-fw-card-desc'><TwoLine text={layer.top.desc} /></p>
              </div>
              <span className='v22alt-fw-card-link' aria-hidden='true' />
              <div className='v22alt-fw-card-item'>
                <div className='v22alt-fw-card-kicker'>Accelerator</div>
                <div className='v22alt-fw-card-name'><TwoLine text={layer.bottom.name} /></div>
                <p className='v22alt-fw-card-desc'><TwoLine text={layer.bottom.desc} /></p>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className='v22alt-fw-pagination' role='tablist' aria-label='Layer navigation'>
          {layers.map((l, i) => (
            <button
              key={l.n}
              type='button'
              ...
              className={`v22alt-fw-dot ${current === i ? 'is-active' : ''}`}
              onClick={() => scrollToCard(i)}
            >
              <span className='v22alt-fw-dot-idx'>{l.n}</span>
            </button>
          ))}
        </div>
      </div>
