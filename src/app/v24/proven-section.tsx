    ...
    <section className='v24-section v24-proven' id='work' data-surface='light'>
      <div className='v24-container'>
        ...
        <div className='v24-proven-header'>
          <div className='v24-proven-header-left'>
            <div className='num'>N°05 / Proven</div>
            <h2>Proven where it <em>matters.</em></h2>
          </div>
          <div className='v24-proven-header-right'>
        ...
        <div className='v24-proven-filters'>
          {(['all', 'industry', 'solution', 'technology'] as Filter[]).map((f) => (
            <button
              key={f}
              className={`v24-proven-filter ${filter === f ? 'active' : ''}`}
              onClick={() => setFilter(f)}
            >
        ...
        {/* Carousel */}
        <div className='v24-proven-carousel'>
          <div className='v24-proven-track' ref={trackRef}>
            {filtered.map((c, i) => (
              <div
                key={c.title}
                className={`v24-proven-card ${i === activeIdx ? 'active' : ''}`}
                onClick={() => setActiveIdx(i)}
              >
                {/* Image area */}
                <div className='v24-proven-img'>
                  <img src={c.image} alt={c.kicker} loading='lazy' />
                  <div className='v24-proven-img-overlay'>
                    <span className='v24-proven-metric'>{c.metric}</span>
                  </div>
                  <div className='v24-proven-img-dots' />
                </div>
                {/* Content */}
                <div className='v24-proven-content'>
                  <span className='v24-proven-kicker'>{c.kicker}</span>
                  <h4>{c.title}</h4>
                  <p>{c.body}</p>
                  {c.techs && (
                    <div className='v24-proven-techs'>
                      {c.techs.map((t) => <span key={t} className='v24-proven-tech-pill'>{t}</span>)}
                    </div>
                  )}
                </div>
              </div>
          ...
          {/* Navigation + CTA row */}
          <div className='v24-proven-nav'>
            <div className='v24-proven-nav-center'>
              <button className='v24-proven-arrow' onClick={prev} aria-label='Previous'>←</button>
              <div className='v24-proven-dots'>
                {filtered.map((_, i) => (
                  <span key={i} className={`v24-proven-dot ${i === activeIdx ? 'active' : ''}`} onClick={() => setActiveIdx(i)} />
                ))}
              </div>
              <button className='v24-proven-arrow' onClick={next} aria-label='Next'>→</button>
            </div>
            <a href='/case-studies' className='v24-proven-all-cta'>
