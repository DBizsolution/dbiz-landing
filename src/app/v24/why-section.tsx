...
function WhyDiagram({ active, onSelect }: { active: number; onSelect: (i: number) => void }) {
  ...
  return (
    <svg viewBox='0 0 440 440' xmlns='http://www.w3.org/2000/svg' className='v24-why-diagram'>
      <defs>
        <filter id='v24-why-glow'>
          <feGaussianBlur stdDeviation='4' result='coloredBlur' />
          <feMerge><feMergeNode in='coloredBlur' /><feMergeNode in='SourceGraphic' /></feMerge>
        </filter>
        <filter id='v24-why-glow-sm'>
          <feGaussianBlur stdDeviation='2' result='coloredBlur' />
          <feMerge><feMergeNode in='coloredBlur' /><feMergeNode in='SourceGraphic' /></feMerge>
        </filter>
        {/* Blueprint textures */}
        <pattern id='v24-why-hatch' patternUnits='userSpaceOnUse' width='6' height='6' patternTransform='rotate(45)'>
          <line x1='0' y1='0' x2='0' y2='6' stroke='rgba(240,123,47,0.15)' strokeWidth='0.8' />
        </pattern>
        <pattern id='v24-why-cross' patternUnits='userSpaceOnUse' width='8' height='8'>
        ...
        <pattern id='v24-why-grid' patternUnits='userSpaceOnUse' width='20' height='20'>
          <path d='M 20 0 L 0 0 0 20' fill='none' stroke='rgba(26,45,90,0.1)' strokeWidth='0.5' />
        </pattern>
        <clipPath id='v24-why-clip'><rect x='12' y='12' width='416' height='416' /></clipPath>
      </defs>

      {/* Frame */}
      <rect x='10' y='10' width='420' height='420' fill='url(#v24-why-grid)' stroke='rgba(26,45,90,0.15)' strokeWidth='1' />
      {/* Corner brackets */}
      ...
      <g clipPath='url(#v24-why-clip)'>
        {/* Crosshatch texture fill behind orbit */}
        <circle cx={cx} cy={cy} r={r + 20} fill='url(#v24-why-cross)' />
        ...
        <circle cx={cx} cy={cy} r={r} fill='none' stroke='rgba(240,123,47,0.18)' strokeWidth='0.8' strokeDasharray='5 4' className='v24-why-orbit' />
        ...
        <path d={`M ${cx} ${cy - r + 50} A ${r - 50} ${r - 50} 0 1 1 ${cx - 0.01} ${cy - r + 50}`} fill='url(#v24-why-hatch)' stroke='none' />
        ...
        {nodes.map((node) => {
          ...
          return (
            <g key={`spoke-${node.idx}`}>
              <line x1={sx} y1={sy} x2={ex} y2={ey} stroke='#F07B2F' strokeWidth={isActive ? 1.5 : 0.8} strokeDasharray='5 4' opacity={isActive ? 0.9 : 0.25} className='v24-why-spoke' />
              <circle r='2' fill='#F07B2F' opacity={isActive ? 0.9 : 0.2} filter='url(#v24-why-glow-sm)'>
        ...
        {nodes.map((node) => {
          const isActive = node.i === active
          return (
            <g key={node.idx} className={`v24-why-node ${isActive ? 'active' : ''}`} onClick={() => onSelect(node.i)} style={{ cursor: 'pointer' }}>
              {isActive && <circle cx={node.x} cy={node.y} r={nodeR + 6} fill='none' stroke='#F07B2F' strokeWidth='0.8' opacity='0.4' strokeDasharray='3 3' className='v24-why-active-ring' />}
              {/* Hatched fill for inactive */}
              {!isActive && <circle cx={node.x} cy={node.y} r={nodeR - 1} fill='url(#v24-why-hatch)' />}
              <circle cx={node.x} cy={node.y} r={nodeR} fill={isActive ? '#F07B2F' : 'rgba(13,27,62,0.85)'} stroke={isActive ? '#F07B2F' : '#1A2D5A'} strokeWidth={isActive ? 2 : 1.5} filter={isActive ? 'url(#v24-why-glow)' : 'none'} className='v24-why-node-circle' />
        ...
        <circle cx={cx} cy={cy} r={hubR + 6} fill='none' stroke='rgba(240,123,47,0.12)' strokeWidth='0.8' strokeDasharray='3 3' className='v24-why-active-ring' />
        <circle cx={cx} cy={cy} r='18' fill='#E06B1F' className='v24-why-hub-pulse' />
...
export default function WhySection() {
  ...
  return (
    <section className='v24-section' id='about' data-surface='light'>
      <div className='v24-container'>
        <div className='v24-why-head'>
        ...
        <div className='v24-why-interactive'>
          {/* Diagram — center */}
          <div className='v24-why-diagram-wrap'>
            <WhyDiagram active={active} onSelect={setActive} />
          </div>

          {/* Detail panel — right */}
          <div className='v24-why-detail' key={active}>
            <span className='v24-why-detail-idx'>{reason.idx} / {reason.label}</span>
            <h3>{reason.title}</h3>
            <p>{reason.body}</p>
            <a href='#cta' className='v24-why-cta'>Learn more about us <span className='arrow'>→</span></a>
            <div className='v24-why-detail-nav'>
              {reasons.map((_, i) => (
                <button
                  key={i}
                  className={`v24-why-detail-dot ${i === active ? 'active' : ''}`}
                  onClick={() => setActive(i)}
